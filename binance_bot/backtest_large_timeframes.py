...
def main():
    print("Iniciando comparativo de timeframes maiores...")
    exchange = ccxt.binance()
    
    # Busca dados cobrindo o mesmo período de aproximadamente 20 dias
    # 1. 2000 candles de 15m (~20.8 dias)
    print("Buscando dados de 15m...")
    candles_15m = exchange.fetch_ohlcv('SOL/USDT', '15m', limit=2000)
    df_15m = pd.DataFrame(candles_15m, columns=['ts', 'open', 'high', 'low', 'close', 'volume'])
    
    # 2. 1000 candles de 30m (~20.8 dias)
    print("Buscando dados de 30m...")
    candles_30m = exchange.fetch_ohlcv('SOL/USDT', '30m', limit=1000)
    df_30m = pd.DataFrame(candles_30m, columns=['ts', 'open', 'high', 'low', 'close', 'volume'])
    
    # 3. 500 candles de 1h (~20.8 dias)
    print("Buscando dados de 1h...")
    candles_1h = exchange.fetch_ohlcv('SOL/USDT', '1h', limit=500)
    df_1h = pd.DataFrame(candles_1h, columns=['ts', 'open', 'high', 'low', 'close', 'volume'])
    
    print("\n==================================================")
    print("  TESTE DE TIMEFRAME MAIOR: 15m vs 30m vs 1h")
    print("        Par: SOL/USDT | Periodo: ~20 dias")
    print("==================================================")
    
    # 15m
    res_15m = run_backtest(df_15m, CONFIG, initial_balance=100.0, trade_amount=14.0)
    print("\n[TIMEFRAME DE 15 MINUTOS]:")
    print(f"   - Lucro Liquido: ${res_15m['profit']:.2f} USDT")
    print(f"   - Taxa de Acerto: {res_15m['win_rate']:.1f}%")
    print(f"   - Total de Trades: {res_15m['total']} (Wins: {res_15m['wins']} | Losses: {res_15m['losses']})")
    
    # 30m
    res_30m = run_backtest(df_30m, CONFIG, initial_balance=100.0, trade_amount=14.0)
    print("\n[TIMEFRAME DE 30 MINUTOS]:")
    print(f"   - Lucro Liquido: ${res_30m['profit']:.2f} USDT")
    print(f"   - Taxa de Acerto: {res_30m['win_rate']:.1f}%")
    print(f"   - Total de Trades: {res_30m['total']} (Wins: {res_30m['wins']} | Losses: {res_30m['losses']})")
    
    # 1h
    res_1h = run_backtest(df_1h, CONFIG, initial_balance=100.0, trade_amount=14.0)
    print("\n[TIMEFRAME DE 1 HORA]:")
    print(f"   - Lucro Liquido: ${res_1h['profit']:.2f} USDT")
    print(f"   - Taxa de Acerto: {res_1h['win_rate']:.1f}%")
    print(f"   - Total de Trades: {res_1h['total']} (Wins: {res_1h['wins']} | Losses: {res_1h['losses']})")
    
    print("\n==================================================")
    print("💡 CONCLUSÃO:")
    profits = {'15m': res_15m['profit'], '30m': res_30m['profit'], '1h': res_1h['profit']}
    best_tf = max(profits, key=profits.get)
    print(f"O melhor timeframe testado foi o de **{best_tf}** com lucro de ${profits[best_tf]:.2f} USDT.")
    print("==================================================")
